...
const OFFICIAL_MODELS: Array<[string, string, number]> = [
  ...
  ["minimax/minimax-m3-free", "MiniMax M3", 1000000],
  ["minimax/minimax-m2.7-free", "MiniMax M2.7", 197000],
...
const vision = (id: string) =>
  /Qwen|Kimi|gemini|gpt-|deepseek-v4-flash-vision|MiniMax-M3|mimo-v2\.5$|muse|grok|fugu|inkling/i.test(
    id,
  );
    ...
    ([id, name, contextWindow]) =>
      [
        id,
        {
          ...
          apiFormat: id.startsWith("claude-") ? ("anthropic" as const) : ("openai" as const),
